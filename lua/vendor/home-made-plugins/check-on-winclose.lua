do
  if vim.g.quickfix_win_loaded == 1 then return end
...
local function create_window_at_top_and_focus(opts)
  ...
  -- todo: use alternative buffer
  local bufnr = nil
  if bufnr == exclude_bufnr then bufnr = nil end

  vim.cmd(("%srightbelow %s"):format(vertical and "vert " or "hor ", bufnr and ("sb " .. bufnr) or "new"))

  vim.schedule(function()
    if opts.height and not opts.vertical then vim.cmd("resize " .. opts.height) end
...
vim.api.nvim_create_augroup("check-on-winclose", { clear = true })
vim.api.nvim_create_autocmd("WinClosed", {
  group = "check-on-winclose",
  callback = function(ctx)
    local curwin = tonumber(ctx.match)
    if not curwin or vim.api.nvim_win_get_config(curwin).relative ~= "" then return end
